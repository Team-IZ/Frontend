...
/*
  ── MG-08 상세 목업은 지웠다(실서버 연결, integration-process ⑤) ──

  이 아래에 팀 편성 5국면·제출·응시·결과 1100여 줄이 있었다. 상세는 이제
  `_/api`가 서버에서 읽는다. **목록(MG-07)이 아직 이 파일을 쓰고 있어** 파일
  자체는 남긴다 — 위쪽 절반이 그 몫이다.

  같이 지운 것: `withParticle`(`@/lib/format`에 같은 함수가 있다) ·
  `deadlinePassed`(리포트 발행 확인이 사라져 쓰는 데가 없다) ·
  `MANAGED_CLASSES`·`COHORT_NAME`은 목록이 아직 써서 남겼지만 **둘 다 목업
  그림에서 역산한 값이다**(A반 26·B반 26·C반 23 · '7기') — 목록을 붙일 때
  `class-progress`와 `useManagerCohort()`로 갈아탄다.
*/
