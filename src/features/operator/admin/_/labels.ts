...
export const ROSTER_ISSUE_LABEL: Record<RosterIssueReason, string> = {
  ...
  */
  ...
  /*
    ⚠ **서버 규칙을 그대로 지킨다.** 서버는 머리글이 **정확히 `이름,이메일` 두 열,
    그 순서**여야 받는다(실측 — 열을 더 두거나 순서를 바꾸면 `CSV_FORMAT_INVALID`).
    화면만 관대하면 「유효 2명」이라 해 놓고 등록에서 통째로 튕긴다.

    이 둘은 행 오류가 아니라 **파일 오류**다 — 한 줄이 아니라 파일을 고쳐야 한다.
  */
  HEADER_NOT_FOUND: '첫 줄이 「이름,이메일」이어야 함 — 그 두 열만 둡니다',
  ENCODING_NOT_UTF8: 'UTF-8이 아님 — 엑셀에서 「CSV UTF-8」로 다시 저장해 주세요',
}
