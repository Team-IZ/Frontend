...
1. **스펙 커버리지가 절반이다.** 프로젝트·교안·세션·리포트·면담이 아직 없다. 그 화면들은
   목으로 돈다. 어댑터가 없으면 **화면이 "생성 훅을 쓰는 것"과 "목을 쓰는 것" 두 세계로 갈린다.**
2. **이미 25개 화면이 이 경계를 지킨다.** 지금 없애면 25개를 건드린다.
3. **되돌릴 수 있다.** 스펙이 전부 나오고 목이 사라지면 어댑터는 한 줄 위임만 남고, 그때
   지우는 것은 기계적이다. 반대(없다가 만들기)는 화면을 다시 짜는 일이다.

> **비용은 인정한다.** 도메인마다 얇은 위임 함수가 생긴다. 그 대가가 화면 25개 재작성보다 훨씬 싸다.
...
| 후보 | 판단 |
|---|---|
| 직접(`useAsync`) | 지금 상태. 캐시·중복제거·무효화·취소를 **화면마다 손으로** |
