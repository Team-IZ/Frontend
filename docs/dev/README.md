...
- [백엔드 API — 22차 요청](backend/backend-api-requests-22.md) — **15차·18차 회신 반영 후 다시 잰 것.** 🔴 `required`인데 `null`인 필드 **25개**(`joinedAt`이 명단을 죽였고 나머지는 아직 안 터진 것) · 🔴 마감 시각을 **생성에서 못 받고 어디서도 못 읽음** · 🔴 교안 등록이 413→**500**으로 바뀜(Lambda 실질 4.5MB) · 🔴 **프록시 동시 요청 502 — 스웨거 UI 흰 화면이 같은 원인** · 🔴 `PLANNED` 회차 현황 무응답 · 🟠 **없는 기수인데 `200`**(교안은 남의 목록 7건을 그대로 줌) · 🟠 `/projects/current`에 총 회차 수 · 🟡 명단에 생긴 회차 축은 어느 화면 것인지. 목록 4.6→**1.9초** ✅ 정렬 ✅ 인원 정의 ✅
