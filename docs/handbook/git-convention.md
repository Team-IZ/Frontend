...
`docs/plan/v2/`에 변경이 생긴 채로 push하면 <https://team-iz.github.io/Frontend/> 가 자동 갱신된다(`.github/workflows/deploy-wireframe.yml`). 발행 루트가 `docs/plan/v2`라 **카탈로그(`wireframe/`)와 정의 문서(`definition/`)가 같이 올라간다** — 카탈로그의 `정의 문서 ↗` 링크가 살아 있으려면 둘이 한 루트여야 한다. 작업 중인 화면도 그대로 공개되므로 팀이 항상 최신을 본다 — 감추고 싶은 단계면 push를 미룬다.
