...
*/
/*
  ⚠️ 여기만 `@/` 별칭이 아니라 상대 경로 + `.ts` 확장자다.
  이 파일은 `npm run check:admin`이 **Node로 단독 실행**한다(`--experimental-strip-types`).
  Node는 우리 별칭을 모르므로 `@/lib/...`로 쓰면 검사가 통째로 깨진다.
*/
...
/*
  ...
  명백히 주소가 아닌 것이다.
*/
export { isEmailShape } from '../../../../lib/validation.ts'
