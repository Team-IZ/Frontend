...
{
  /*
    **이슈 265 — 실측해봤더니 계산값을 못 정한다는 게 결론이었다.**

    `registerCurriculum`이 App Runner origin으로 직접 쏘도록 바뀌면서 옛 Lambda 4.5MB
    벽을 더 이상 안 지난다. 그 벽 기준의 정밀한 상·하한 검산(4,600,000~4,717,187바이트)은
    이제 이 경로에 안 맞는다 — 배포본에서 실측해보니 15~25MB 구간이 반복 대용량 업로드로
    App Runner가 불안정해지는 영향을 받아 같은 크기도 성공·서버 500이 왔다 갔다 했다
    (`uploadLimits.ts` 주석 · `decision-log.md` 참고).

    그래서 여기서는 "정확한 값"이 아니라 "말이 되는 범위인가"만 지킨다 — 옛 4.5MB보다는
    커야 하고(그래야 우회가 의미 있다), 화면이 못 쓸 만큼 크지는 않아야 한다(브라우저가
    통째로 메모리에 물고 있는 파일이다). 실제 성공 여부는 항상 서버가 결정하고, 화면의
    재시도 안내가 실패를 받는다 — 이 상수는 그 이전에 명백한 오조작만 거른다.
  */
  assert.ok(MAX_UPLOAD_BYTES > 4_718_592, '옛 Lambda 상한보다 커야 우회가 의미 있다(이슈 265)')
  assert.ok(
    MAX_UPLOAD_BYTES <= 100 * 1024 * 1024,
    '100MB 넘게 브라우저가 파일을 통째로 물지 않는다',
  )
  assert.strictEqual(
    MAX_UPLOAD_LABEL,
