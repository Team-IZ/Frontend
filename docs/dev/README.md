...
- [백엔드 API — 29차 요청](backend/backend-api-requests-29.md) — **25차 회신 열한 건 실서버 확인 + 우리가 틀린 것 정정.** 🚨 **404만 무응답이다** — 「형식 400이 오니 프록시 층이 아니다」가 틀렸고, DB를 안 타는 없는 경로도 멈춘다(앞단이 **상태 코드**를 보고 가른다) · 🟡 반 이름에 「반」이 없는 행. ✅ `status` 배열 · 재분석 409 · `NULLS LAST` · **반 이동 한 번에** · 교안 삭제 · enum 5값
