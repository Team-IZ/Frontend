...
> 화면에 반영했습니다(§6).
>
> 🔴 **다만 그 뒤 배포에서 `api:check` error가 다시 1건 생겨 저희 CI가 막혀 있습니다**(§1).
> 교육생 화면과는 무관한 스키마인데, 검사는 스펙 전체를 보기 때문에 **연동 작업 전체를
> 푸시하지 못하고 있습니다.**
>
> 나머지는 **테스트 데이터**입니다. 교육생 3화면을 전수 점검하려고 상태 조합을
> 정리했는데, **지금 계정으로 재현할 수 있는 것이 절반도 안 됩니다.** 화면은 다
> 만들어져 있지만 **한 번도 눈으로 본 적 없는 분기가 대부분**입니다.
>
> 그리고 점검하다 **저희 쪽 버그를 하나 찾아 고쳤습니다**(§6) — 같은 실패를 두 화면이
> 다르게 말하고 있었습니다.
...
| | 무엇 | 우선순위 |
|---|---|---|
| **R1** | `TraineeTimelineEvent.type` — **`api:check` error 1건 · 저희 CI가 막힙니다** | 🔴 **푸시를 못 합니다** |
| **R2** | **상태 재현용 계정/데이터** — 지금 카드 상태 10종 중 **2종**만 볼 수 있습니다 | 🔴 검증이 막힙니다 |
| **R3** | 새 BFF 경로가 `/api/v0/api/v0/…`로 **중복**돼 있습니다 · 정본이 어느 쪽인가요 | 🟡 확인 |
| **R4** | 같은 회차를 홈은 `미응시`, 리포트는 `발행 전`이라고 합니다 | 🟡 확인 |

---

## 1. 🔴 R1 — `api:check` error 1건이 저희 CI를 막고 있습니다

방금 받은 스펙(`sha256 fd1d7beb…` · 오퍼레이션 134)에서 error가 다시 생겼습니다.
...
`GET /cohorts/{cohortId}/trainees/{traineeId}/timeline`을 `사용 가능`으로 열어 주시면서
들어온 것으로 보입니다.

### 저희 화면은 이 스키마를 쓰지 않습니다

교육생 화면과 무관한 매니저용 스키마입니다. **그런데도 막힙니다** — `api:check`는
스펙 전체를 검사하고, CI가 그 종료코드로 통과를 판정하기 때문입니다.

```
CI "API spec" 단계:  npm run api:check  →  error 1건이면 종료코드 1  →  빌드 실패
```

**교육생 3화면 연동을 전부 마쳐 커밋까지 끝냈는데 푸시를 못 하고 있습니다.**
저희 쪽 검사 11개 중 나머지 10개(포맷·디자인 토큰·린트·코드젠 드리프트·빌드)는
전부 통과합니다.

### 요청

19차 R1 · 23차 R1 · 26차 R2와 **같은 자리**입니다. 그때처럼 둘 중 하나면 됩니다.

- 설명문이 사실이다 → `@Schema(nullable = true)`
- 실제로는 `null`이 안 온다 → **설명문에서 그 단어를 빼 주세요**

26차 R2 때 *"이웃 필드를 설명하던 문장이 검사기에 걸렸다"* 는 것과 같은 유형이면
설명문 수정이 맞습니다.

> **이것만 풀리면 바로 푸시합니다.** 나머지 요청들은 급하지 않습니다.

---

## 2. 🔴 R2 — 상태 대부분을 한 번도 못 봤습니다
...
## 5. 🟢 곁들여 — 주신 명단으로는 세션을 테스트할 수 없습니다
...
R2의 ① `trainee-ready`(`ASSESSMENT_AVAILABLE`) 계정이 있으면 그 문제까지 함께
풀립니다 — **분석이 끝난 계정 하나가 홈·제출·세션 셋을 동시에 열어 줍니다.**

---

## 6. 저희 쪽에서 찾아 고친 것 — 같은 실패를 두 화면이 다르게 말했습니다
...
**화면은 다 만들어져 있고 커밋도 끝냈습니다.** 남은 것이 둘입니다.

1. **R1** — `api:check` error 1건 때문에 **푸시를 못 하고 있습니다.** 한 줄이면 풀립니다
2. **R2** — *"만들어 둔 분기가 실제로 맞게 그려지는가"* 를 확인할 데이터가 없습니다

**R1이 제일 급합니다.** R2는 ①(정상 흐름 3종)만 주셔도 나머지를 한 번에 점검하고
결과를 알려 드리겠습니다.
