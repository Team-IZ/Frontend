...
/*
  ...
  실측으로 로그인 진입에서 60초 넘게 아무 응답이 없었다 — 그동안 조회는 `isPending`에
  머물고, 그것을 그리는 화면은 **끝나지 않는다.**

  ⚠ 한때 「이 백엔드는 없는 것을 물으면 404가 아니라 무응답」이라고 적어 뒀는데
  (18차 R1·R7 · 21차 · 25차 R4), **그건 Lambda 호스트 쪽 현상이었다** — App Runner로
  옮긴 뒤 5종 전부 404가 0.5~0.8초에 온다(38차 §6). 예산은 그대로 둔다: 이유가
  콜드스타트로 바뀌었을 뿐, `fetch`에 기본 타임아웃이 없다는 사실은 그대로다.
