...
export type Manager = {
  ...
  email: string
  /**
   * 담당 반. **기수를 달고 다닌다** — 배정이 기간형 이력이라 `6기 A반 담당이 누구였나`가
   * 이 계정에 붙어 있고, 지우면 지난 기수의 기록이 끊긴다(OP-06 §3).
   */
  assignment: ManagerAssignment | null
  /** 담당 인원 합. **서버가 센다** — 반 목록 전량을 받아야만 셀 수 있는 값이다 */
...
export type ManagerQuery = {
  search?: string
  /** 담당 반이 그 기수에 있는 매니저만. 매니저 탭 자체는 **기관 전체**다 */
  cohortId?: string
  status?: ManagerStatus
}
...
export type InviteManagerRequest = {
  email: string
  /** 담당 반 · 선택 · 나중에 배정 가능. **권한 선택이 없다** — 매니저는 한 종류다 */
  classId: string | null
}
...
export type CostSummary = {
  ...
  /** 기관 전체 이번 달 사용액(USD) */
  total: number
  /** 계약 예산. 없으면 null — 비율을 그리지 않는다 */
  budget: number | null
  /** 전월 대비 증감(%). 부호를 그대로 쓴다 */
  changePct: number
  /** 기수별 배분. 1인당은 화면이 나눈다(같은 줄에 분자·분모가 다 있다) */
  cohorts: CohortCost[]
}
