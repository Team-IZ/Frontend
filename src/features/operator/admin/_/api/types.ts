...
export type Trainee = {
  ...
  /*
    ⚠ **`team`과 `previousCohort`를 뺐다**(decision-log D29). 목업에는 둘 다 있지만
  ...
  */
   ...
   * 요구하는데, 그 액션이 어느 오퍼레이터 화면에도 없었다(D30-①).
   *
...
export type Manager = {
  ...
  email: string
  /**
   * **소속 기수.** 매니저도 반처럼 기수에 붙는다(decision-log D38).
   *
  ...
  cohortIds: string[]
  /**
   * 담당 반 — 기수마다 한 묶음. **조회 범위만큼 담긴다**(decision-log D37).
   *
   ...
   *
   * ⚠ 끝난 기수까지 더하고 있었다(D36). 5·6·7기를 맡은 사람이 `73명`으로 보였는데
   ...
   *
   * 반의 `assignedAt`/`assignedBy`(D25)와 같은 모양이다.
   */
  invitedAt: string
  invitedBy: string
}
...
export type ManagerQuery = {
  search?: string
  /**
   * **이 기수 소속만**(D38). 화면이 늘 채워 보낸다 — 매니저 탭이 상단 기수 스위처를
...
export type InviteManagerRequest = {
  email: string
  /**
   * 어느 기수 매니저로 초대하는가(D38).
   *
   * **반이 아니라 기수다.** 가입 전에는 로그인을 못 해 반의 면담·독촉을 처리할 수 없으므로
   * 담당 반은 안 받지만(D34), **기수 소속은 정할 수 있다** — 그래야 초대 대기 계정이
 ...
 * 비용. **여기서는 절대 금액을 쓴다.**
 *
 * OP-01 대시보드가 증감만 쓰는 것과 다르다 — 거기는 *"운영이 정상인가"* 를 보는
 * 지표판이라 판단 기준 없는 금액이 놀라움만 주지만, 이 탭은 **기관 총량과 기수/반
 * 배분**이 질문이라 금액이 그 답이다(OP-01 §5가 `비용 → OP-06 비용`으로 보낸 이유).
 */
export type CostSummary = {
  ...
  total: number
  /**
   * 지난달 사용액. **증감만으로는 판단이 안 된다**(decision-log D39) — `+12%`가
   * `$400 → $412`인지 `$50 → $56`인지에 따라 할 일이 다르다. 첫 달이면 null.
   */
  previousTotal: number | null
  /**
   * **기수 시작부터 지금까지 누적**(D40).
   *
   * 이 화면의 범위는 기수인데(정의서 §3) 이번 달만 보고 있었다 — 기수가 7개월이라
   * *"지금까지 얼마 썼나"* 와 *"예산이 버티나"* 를 답할 수 없었다. 예산 비율도 월이
   * 아니라 이 값으로 잰다.
   */
  cohortTotal: number
  /** 계약 예산(기수 전체). 없으면 null — 비율을 그리지 않는다 */
  budget: number | null
  /** 남은 개월 수. 예산 소진율과 같이 봐야 판단이 된다 */
  monthsLeft: number
  /** 전월 대비 증감(%). 부호를 그대로 쓴다 */
  changePct: number
  /**
   * **기수 시작월부터 이번 달까지**, 최근이 앞으로(D40).
   *
   * 월별이 이 화면의 새 축이다 — 한 달만 보면 `$268`이 많은지 적은지 알 수 없고,
   * **어느 달에 튀었는지**도 못 본다.
   */
  monthly: MonthlyCost[]
  /**
   * 그달에 **실제로 비용이 발생한 기수**만. 평시에는 하나, 전환기에는 둘이다.
   *
   * 부트캠프는 한 번에 한 기수를 돌리므로 여러 기수가 상시 공존하지 않는다 —
   * 목이 두 기수를 통째로 겹쳐 놓아 화면에 늘 둘이 보였다(D39).
   */
  cohorts: CohortCost[]
}
 ...
 * 한 달치 비용.
 *
 * **`projectNames`가 이 타입의 핵심이다**(D40). 금액만 있으면 `$268`이 많은지 적은지
 * 판단할 수 없는데, **비용은 세션에서 나오고 세션은 회차에서 나온다** — 회차가 둘인 달이
 * 비싼 것은 정상이고, 하나인데 비싼 달이 조치 대상이다.
 *
 * 회차 **이름 문자열만** 받는다 — 프로젝트 도메인 객체를 들이면 레이어 규칙을 넘는다
 * (D15). 교안의 `linkedProjectNames`가 같은 방식이다.
 */
export type MonthlyCost = {
  /** `2026-07` */
  month: string
  amount: number
  /** 세션 수 — **비용의 원인**이라 금액 옆에 둔다 */
  sessions: number
  /** 그달에 마감된 회차 이름. 비어 있으면 회차 없이 재시험만 있던 달이다 */
  projectNames: string[]
}

export type CohortCost = {
  id: string
  name: string
  amount: number
  /*
    ⚠ **1인당을 화면이 만들지 않는다**(D40). 학생 1명이 쓰는 세션 규격이 고정이고
    (`문항 3 · 단계 4 · 힌트 2 · 세션 70분` — 전 기수 공통, OP-04) **모든 반이 같은
    회차를 한다**(OP-03 `10반 250명이 같은 회차를 한다`). 그래서 1인당 비용은 구조적으로
    상수에 수렴한다 — 실측으로도 $1.12~$1.23이라 차이가 판단 근거가 못 된다.
    인원은 남긴다: 규모를 아는 데는 쓰인다.
  */
  trainees: number
  /**
   * 기간 표기(`2026-03 ~ 09`).
   *
   * **왜 이 기수가 이 달에 있는지**를 카드가 스스로 말해야 한다 — 전환기에 둘이 뜰 때
   * 기간이 없으면 *"왜 두 기수가 같이 있지"* 를 묻게 된다.
   */
  period: string
}
...
export type ClassCost = {
  classId: string
  /** 이 반이 속한 기수 — 조회가 이 값으로 거른다(D39) */
  cohortId: string
  className: string
  managerName: string | null
  /**
   * **월별 비용 — 오래된 달이 앞**(decision-log D43).
   *
   * 한 달치만 주고 있었다. 그러면 *"6월에 어느 반이 많이 썼나"* 를 볼 수 없고, 달을
   * 골라 가며 봐도 **두 달을 나란히 비교할 수 없다** — 앞 달 숫자를 외워야 한다.
   *
   * 아직 안 지난 달은 담지 않는다 — 기수가 9월까지여도 7월이면 다섯 칸이다.
   */
  monthly: MonthlyClassCost[]
  /**
   * 기수 누적 — **행 끝의 결론**이다.
   *
   * 월별 칸이 *언제* 를 말한다면 이 값은 *총량* 을 말한다. 한 달치는 표본이 작아
   * 우연히 갈리지만(실측 폭 $4) 누적은 $20 넘게 벌어진다(D41).
   */
  cohortAmount: number
 ...
 * 반별 비용 정렬(D43).
 *
 * **둘만 둔다.** 월이 열로 펼쳐졌으므로 *어느 달에 누가 많이 썼나* 는 **눈으로 훑는다** —
 * 달마다 정렬 옵션을 만들면 일곱 개가 되고, 그건 매트릭스가 이미 하는 일이다.
 */
export type ClassCostSort = 'NAME' | 'COHORT_AMOUNT'
