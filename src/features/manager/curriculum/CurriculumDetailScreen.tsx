...
export default function CurriculumDetailScreen() {
  ...
  */
  ...
  if (!head.data && !head.isError) {
    /*
      ⚠ **없는 `materialId`는 이제 404가 온다** — `CURRICULUM_MATERIAL_NOT_FOUND`가
      0.5초에 오고 아래 「교안을 찾을 수 없습니다」로 정확히 떨어진다(38차 §6 실측).
      한때 「404가 아니라 매달린다」고 적어 두고 30초에서 끊었는데(32차 R12),
      **그건 Lambda 호스트 쪽 현상**이었다.

      `SlowNotice`는 그대로 둔다 — 이유가 바뀌었을 뿐 필요는 남는다. **잠든 서버는
      깨는 데 76초**라(`Loading.tsx`) 정상 요청도 첫 진입에서 오래 걸릴 수 있고,
      그때 12초에 사정을 말해 주는 편이 낫다.
