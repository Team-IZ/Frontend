...
/*
  1~4단은 "…까지"로 **어디까지 올라갔나**를 말한다. 0단은 그 계단에 못 올라선 것이라
  같은 어법을 쓸 수 없다.

  ⚠ **0단 문구는 기획에 없다 — 프론트가 정한 값이다.** 세션 화면이 같은 상황에서
  쓰는 말("설명이 안 닿아 접힌 경우", TransitionScreen)을 빌렸다. 학생이 보는
  문구이므로 "실패"·"0점" 같은 말은 쓰지 않는다(A5 — 점수는 비노출).
  기획에서 값이 나오면 여기만 바꾼다.
*/
