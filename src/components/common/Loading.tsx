 ...
 *
 * 그래도 안내는 필요하다 — **잠든 서버는 깨는 데 최대 76초**라(`client.ts` 예산 주석)
 * 정상 요청도 첫 진입에서 오래 걸린다. 스켈레톤만 있으면 **영원히 곧 올 것처럼** 보인다.
 *
 * ⚠ 한때 「없는 회차는 서버가 404 대신 매달린다」(18차 R7·R8)를 이유로 적어 뒀는데,
 * **그건 Lambda 호스트 쪽 현상이었다**(38차 §6 — App Runner는 5종 전부 404를 0.5~0.8초에
 * 준다). 이유가 바뀌었을 뿐 콜드스타트 때문에 이 안내는 그대로 필요하다.
 *
