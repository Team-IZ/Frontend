...
{
  /*
    **TEMP(이슈 #265) — 실측 전까지는 계산값이 아니라 상식적인 범위만 검산한다.**

    `registerCurriculum`이 App Runner origin으로 직접 쏘도록 바뀌면서 옛 Lambda 4.5MB
    벽을 더 이상 안 지난다(`uploadLimits.ts` 주석 참고) — 그래서 그 벽 기준의 정밀한
    상·하한 검산(4,600,000 ~ 4,717,187바이트)은 지금 이 경로에 안 맞는다.

    실측이 끝나면 이 자리를 새 근거(이분 탐색 경계값)로 다시 채운다. 그때까지는 "말이
    되는 범위인가"만 지킨다 — 옛 4.5MB보다는 커야 하고(그래야 우회가 의미 있다),
    화면이 못 쓸 만큼 크지는 않아야 한다(브라우저가 통째로 메모리에 물고 있는 파일이다).
  */
  assert.ok(MAX_UPLOAD_BYTES > 4_718_592, '옛 Lambda 상한보다 커야 우회가 의미 있다(TEMP #265)')
  assert.ok(
