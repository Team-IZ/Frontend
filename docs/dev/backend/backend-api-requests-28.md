...
> **26차 반영을 확인했습니다.** `api:check` **error 0건**이고, `NOT_STARTED`도 실계정에서
> 나옵니다. 응시 창 교집합은 화면에 반영했습니다(§4).
>
> 이번 요청은 **테스트 데이터 하나**입니다. 교육생 3화면을 전수 점검하려고 상태 조합을
> 정리했는데, **지금 계정으로 재현할 수 있는 것이 절반도 안 됩니다.** 화면은 다
> 만들어져 있지만 **한 번도 눈으로 본 적 없는 분기가 대부분**입니다.
>
> 그리고 점검하다 **저희 쪽 버그를 하나 찾아 고쳤습니다**(§3) — 같은 실패를 두 화면이
> 다르게 말하고 있었습니다.
...
| **R2** | 같은 회차를 홈은 `미응시`, 리포트는 `발행 전`이라고 합니다 | 🟡 확인 |
...
## 3. 저희 쪽에서 찾아 고친 것 — 같은 실패를 두 화면이 다르게 말했습니다
...
**화면은 다 만들어져 있습니다.** 남은 것은 *"만들어 둔 분기가 실제로 맞게
그려지는가"* 인데, 그것을 확인할 데이터가 없습니다.

R1(①만이라도)을 주시면 정상 흐름 전체를 한 번에 점검하고 결과를 알려 드리겠습니다.
