...
| `Curriculum`·`SectionIndex`·`SectionTopic`·`ProjectCurriculumScope` | 교안·섹션·섹션 주제(AI 기본값+사람 편집)·프로젝트 교안 범위 한정(선택) | CUR-01/02/03·D103 | 플로우 G 참조 (섹션 의미 정체성 앵커·source 감사전용·범위 부재=기수 전체) | 버전 이력 / 주제 속성(변경은 AuditLog) / Scope 부재=무제한 |
| `Cohort`·`Class`·`Team`·`Trainee`·`Project`·`측정계획(회차·due_at)` | 조직·일정 골격 | ORG/PLAN | 02-domain-hierarchy 참조 | 이력 보존 |

> ⚠ **명세 공백:** `참여도`·`팀 진행률`의 원천 엔티티·산출 정의(분자/분모)가 기능명세서에 없음. 아래 플로우 F에서 미완성으로 표기 — **백엔드 정의 선행 필요.** (교안 매핑의 신뢰도·임계값 공백은 D101~D103에서 섹션 주제 모델로 전환하며 소멸 — 더 이상 해당 없음.)
...
## 플로우 G — 교안 주제 → 리포트 위치 안내 (섹션 주제 라벨 + 기수 전체 검색, D102·D103)

> SC-M12 교안 관리의 데이터 계약. **매칭은 두 단계로 분리된다:** ① 교안 쪽 = "각 섹션이 무슨 **주제**를 다루나"(교안 내재 속성, 프로젝트 무관) ② 리포트 시점 = "학생이 막힌 주제를 가진 섹션을 검색"(범위 = 기본은 기수 전체 교안, 예외적으로 프로젝트가 한정하면 그 교안). D96·D100의 신뢰도 노출 모델과 D101 전량 수동은 모두 철회됨(경과: D102). **섹션 주제는 AI가 추출 키워드로 기본값을 채우고 총괄이 틀린 것만 편집**(자동완성 패턴, 확신 수준 화면 비노출). 학생별 정확 페이지는 **리포트 시점 검색**(D94 유지).

```
[교안 등록·자동 추출]                    [AI 기본값 → 사람 편집]                    [리포트 소비 · 기수 전체 검색]
SC-M12 등록(CUR-03)
  → Curriculum 저장(상태머신·버전)
      → CUR-01 구조 추출
          → SectionIndex 저장(섹션 의미 정체성·페이지범위·추출 키워드)  ★앵커=heading+임베딩
              → CUR-02 교안 구성: 추출 키워드로 SectionTopic 초안 자동 생성(source=ai_suggested)
                   → 총괄이 섹션별 주제 칩 훑고 틀린 것만 추가/삭제(수정 시 source=manager_edited)
                 → SectionTopic 저장(sectionId·topic·source) — 확률 필드 없음, 화면 비노출
                 → (선택) ProjectCurriculumScope: 프로젝트가 특정 교안으로 안내 범위 한정
                      비면 = 기수 전체 교안이 대상(기본·권장)
                 → MEAS-07 리포트: 학생이 (주제,축) 취약 →
                      [범위: 한정 교안 or 기수 전체] 안에서 그 주제를 가진 SectionTopic 검색
                      → 매칭 섹션의 축→노드타입 규칙(D94) 위치로 안내
                      (주제 매칭 0건 = 안내 생략, 정상 — 억지로 안 채움)
```

### 두 단계 분리가 핵심 (D103)

- **교안 쪽(SectionTopic) = 프로젝트 무관.** "컨테이너 통신 섹션은 Service·Ingress를 다룬다"는 교안의 사실이라 교안 하나당 한 번만 정하면 프로젝트 몇 개에 쓰이든 재사용. 프로젝트마다 반복하지 않음.
- **매칭 = 리포트 시점.** 학생이 실제로 막힌 주제로 그때 검색. 사전에 "프로젝트 기술영역 ↔ 섹션"을 미리 이어둘 필요 없음(그게 D96~D102의 낭비였음).
- **범위 = 기본 기수 전체.** 링크(ProjectCurriculumScope)를 안 걸면 기수 전체 교안에서 검색 — **미링크가 "안내 없음"이 아니라 "전체 검색"**(D103 기본값 뒤집기, 침묵 실패 제거). 교안은 기수 자산(D85)이라 기수가 자연 경계.
- **링크 = 선택적 좁히기.** 프론트/백엔드 혼재 기수에서 특정 프로젝트를 특정 교안으로만 한정하고 싶을 때만. 노동이 아니라 예외 도구.

### 커버리지 = 기수 전체 기준 자동 계산 (매니저 액션 없음)

"이 기수 프로젝트가 측정하는 주제 N개 중 M개를 현재 교안들이 커버" — 미커버 주제는 **교안을 더 올려야 한다는 신호**(프로젝트별 갭보다 유용). 교안 상세·프로젝트 개요 양쪽에 읽기 표시, 사람이 지정하는 값 아님.

### 삭제·불채택 (D96~D101 잔재 최종 정리)

`CurriculumMapping`·confidence·link_state·content_match·source_confidence(D96·D100), `MappingOutcome`(D100), `MappingOverride`(정답 라벨), 사전 "기술영역↔섹션 태그"(D101·D102의 `SectionTag`) 전부 불채택. 임계값 [가정값]·골든셋 문제 소멸. **화면엔 신뢰도·확률·"AI 제안" 배지 없음** — `source`는 감사 로그 전용, UI 비노출(노출하면 D96 재발).
...
| `SectionTopic` | 섹션이 다루는 주제 1건 | CUR-02 | sectionId·topic·**source**(ai_suggested/manager_edited, 감사 전용·화면 비노출) | 단순 속성(변경 이력은 전역 `AuditLog`) |
| `ProjectCurriculumScope` | 프로젝트 교안 범위 한정(선택) | CUR-02 | projectId·curriculumId | **없으면 기수 전체가 기본**(레코드 부재 = 무제한) |

### Type-1 데이터 계약 1건 — 되돌리기 비쌈, ERD 전 확정 (D100에서 판단, D101~D103에서도 유지)

**주제 앵커 = 페이지 번호가 아니라 섹션 의미 정체성.** `SectionTopic`은 `SectionIndex.section_identity`(heading 텍스트 + 청크 임베딩 centroid)를 FK로 가리키고, page_range는 파생 표시값. 버전 재추출 시 identity가 ~동일하면 주제 유지(페이지만 갱신), 섹션이 소멸·모호 분할되면 해당 주제 무효화·재지정(이때도 AI가 재추출 섹션에 다시 기본값 제시). **매칭 방식·범위 정책과 무관하게 이 앵커는 필요.**

### AI 기본값 생성 로직 (구현 참고, 화면 스펙 아님)

키워드 겹침(섹션 keywords ↔ 기술영역명) + 임베딩 유사도로 최상위 후보 산출. 임계값 이상이면 자동 체크, 미만이면 빈 칸(강제로 하나 골라 채우지 않음 — 틀린 걸 채우는 것보다 빈 칸이 안전, D91 미커버=정상 원칙과 동일). 이 로직의 정밀도는 배포 후 `source=ai_suggested`인데 곧이어 `manager_edited`로 바뀐 태그 비율로 오프라인 모니터링 가능(화면 기능 아님, 운영 지표).

> **미해결(BE 정의 선행):** AI 기본값 자동 체크 임계값 자체는 [가정값](실 데이터 없어 초기값 보수적으로). 그 외 신뢰도·산출식 노출 문제는 D101에서 이미 소멸.
...
4. **피드백 루프** = `InterventionOutcome` → `DASH-03` R (플로우 B) — 순환 아님, 회차 경계로 분리. (교안 매핑은 회복 루프도 화면 노출 신뢰도 루프도 없음·D100→D103으로 전부 철회 — `SectionTopic`은 AI가 기본값을 채우고 사람이 편집하는 섹션 속성, 매칭은 리포트 시점 검색.)
5. **교안 주제 앵커(Type-1)** = `SectionTopic`은 페이지가 아니라 `SectionIndex.section_identity`를 FK로 참조(플로우 G). 버전 재추출은 identity diff로 자동 재앵커 — ERD에서 page를 주제 키로 삼으면 안 됨.
6. **교안 범위 기본값(D103)** = `ProjectCurriculumScope` 레코드가 **없으면 기수 전체 교안이 대상**(무제한). ERD에서 "미링크=안내 없음"으로 오해하면 안 됨 — 부재는 무제한이지 빈 집합이 아님.
7. **미해결(설계 전 확정 필요)** = 참여도·팀 진행률 정의, policy_version 관리 주체, 쿨다운/타이밍 상수 테이블(D-5). (교안 매핑 신뢰도·임계값 공백은 D101~D103으로 소멸.)
