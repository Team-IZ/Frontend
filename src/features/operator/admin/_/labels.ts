...
export const ROSTER_ISSUE_LABEL: Record<RosterIssueReason, string> = {
  ...
  */
  ...
  /*
    ⚠ **한때 이 오류가 없었다.** 이름이 비면 화면이 이메일 앞부분으로 채워 「유효」로
    셌는데(*"초대는 나가야 하고 이름은 받는 사람이 고친다"*), **서버는 그 행 하나 때문에
    파일 전체를 거절한다** — `400 TRAINEE_NAME_INVALID · "5행의 이름을 입력해야 합니다"`.
    화면이 `✓ 유효 2명`이라고 해 놓고 누르면 아무도 안 들어갔다. 서버 판정에 맞춘다.
  */
  NAME_REQUIRED: '이름이 비어 있음',
  /*
    ⚠ **서버 규칙을 그대로 지킨다.** 서버가 넓어져서 이 문구도 같이 넓혔다(29차 회신 Q1) —
    이제 **열 순서와 개수를 안 따지고** 인코딩도 UTF-8·CP949 둘 다 받는다. 남은 조건은
    「두 열이 **있는가**」 하나다.

    이 둘은 행 오류가 아니라 **파일 오류**다 — 한 줄이 아니라 파일을 고쳐야 한다.

    ⚠ `ENCODING_NOT_UTF8`은 **이름을 안 바꿨다.** 저장된 값이 아니라 코드 안 식별자라
    바꿔도 얻는 것이 없고, 쓰는 곳이 늘면 그때 정리하면 된다. 문구는 사실에 맞췄다 —
    UTF-8·CP949 **둘 다 아닌** 파일만 여기로 온다.
  */
  HEADER_NOT_FOUND: '첫 줄에 「이름」·「이메일」 열이 있어야 합니다 — 순서·다른 열은 상관없습니다',
  ENCODING_NOT_UTF8: '읽을 수 없는 인코딩 — 엑셀에서 「CSV」 또는 「CSV UTF-8」로 저장해 주세요',
}
