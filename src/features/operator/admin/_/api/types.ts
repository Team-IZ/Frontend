...
export type CostSummary = {
  ...
  total: number
  /**
   * 지난달 사용액. **증감만으로는 판단이 안 된다**(op-06-admin.md OP06-16) — `+12%`가
   * `$400 → $412`인지 `$50 → $56`인지에 따라 할 일이 다르다. 첫 달이면 null.
   */
  previousTotal: number | null
  /**
   * **기수 시작부터 지금까지 누적**(OP06-17).
   *
  ...
  changePct: number
  /**
   * **기수 시작월부터 이번 달까지**, 최근이 앞으로(OP06-17).
   *
   ...
   * 목이 두 기수를 통째로 겹쳐 놓아 화면에 늘 둘이 보였다(OP06-16).
   */
  cohorts: CohortCost[]
}
 ...
 * 한 달치 비용.
 *
 * **`projectNames`가 이 타입의 핵심이다**(OP06-17). 금액만 있으면 `$268`이 많은지 적은지
...
export type CohortCost = {
  ...
  amount: number
  /*
    ⚠ **1인당을 화면이 만들지 않는다**(OP06-17). 학생 1명이 쓰는 세션 규격이 고정이고
...
export type ClassCost = {
  classId: string
  /** 이 반이 속한 기수 — 조회가 이 값으로 거른다(OP06-16) */
  cohortId: string
  className: string
  managerName: string | null
  /**
   * **월별 비용 — 오래된 달이 앞**(op-06-admin.md OP06-20).
   *
   ...
   * 우연히 갈리지만(실측 폭 $4) 누적은 $20 넘게 벌어진다(OP06-18).
   */
  cohortAmount: number
